/*
 * A hwmon driver for the Accton dcs6500 56x fan
 *
...
#define DRVNAME "dcs6500_48z8c_fan"
...
static struct dcs6500_48z8c_fan_data *dcs6500_48z8c_fan_update_device(struct device *dev);
...
static struct attribute *dcs6500_48z8c_fan_attributes[] = {
...
static int dcs6500_48z8c_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int dcs6500_48z8c_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u8 is_fan_fault(struct dcs6500_48z8c_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
                          ...
                          const char *buf, size_t count)
{
    ...
    struct dcs6500_48z8c_fan_data *data = i2c_get_clientdata(client);
    int error, value;
    ...
    data = dcs6500_48z8c_fan_update_device(dev);
                              ...
                              const char *buf, size_t count)
{
    ...
    dcs6500_48z8c_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    dcs6500_48z8c_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
...
static int _find_lm75_device(struct device *dev, void *data)
{
    ...
    struct dcs6500_48z8c_fan_data *prv = data;
                              ...
                              const char *buf, size_t count)
{
    ...
    error = dcs6500_48z8c_fan_write_value(client, reg, value);
...
static const struct attribute_group dcs6500_48z8c_fan_group = {
    .attrs = dcs6500_48z8c_fan_attributes,
};
...
static struct dcs6500_48z8c_fan_data *dcs6500_48z8c_fan_update_device(struct device *dev)
{
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = dcs6500_48z8c_fan_read_value(client, fan_reg[i]);
...
static int dcs6500_48z8c_fan_probe(struct i2c_client *client,
                                const struct i2c_device_id *dev_id)
{
    struct dcs6500_48z8c_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct dcs6500_48z8c_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &dcs6500_48z8c_fan_group);
    if (status) {
        goto exit_free;
    }

    data->hwmon_dev = hwmon_device_register_with_info(&client->dev, "dcs6500_48z8c_fan",
                                                      NULL, NULL, NULL);
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &dcs6500_48z8c_fan_group);
exit_free:
    kfree(data);
exit:
...
static int dcs6500_48z8c_fan_remove(struct i2c_client *client)
{
    struct dcs6500_48z8c_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id dcs6500_48z8c_fan_id[] = {
    { "dcs6500_48z8c_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, dcs6500_48z8c_fan_id);

static struct i2c_driver dcs6500_48z8c_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = dcs6500_48z8c_fan_probe,
    .remove       = dcs6500_48z8c_fan_remove,
    .id_table     = dcs6500_48z8c_fan_id,
    .address_list = normal_i2c,
};

static int __init dcs6500_48z8c_fan_init(void)
{
    return i2c_add_driver(&dcs6500_48z8c_fan_driver);
}

static void __exit dcs6500_48z8c_fan_exit(void)
{
    i2c_del_driver(&dcs6500_48z8c_fan_driver);
}

module_init(dcs6500_48z8c_fan_init);
module_exit(dcs6500_48z8c_fan_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("dcs6500_48z8c_fan driver");
MODULE_LICENSE("GPL");
