			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = dcs6500_48z8c_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void dcs6500_48z8c_cpld_add_client(struct i2c_client *client)
{
...
static void dcs6500_48z8c_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int dcs6500_48z8c_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct dcs6500_48z8c_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct dcs6500_48z8c_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    switch (data->type) {
    case dcs6500_48z8c_cpld1:
        group = &dcs6500_48z8c_cpld1_group;
        break;
    ...
    dcs6500_48z8c_cpld_add_client(client);
    return 0;
...
static int dcs6500_48z8c_cpld_remove(struct i2c_client *client)
{
    struct dcs6500_48z8c_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    dcs6500_48z8c_cpld_remove_client(client);
...
int dcs6500_48z8c_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = dcs6500_48z8c_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(dcs6500_48z8c_cpld_read);

int dcs6500_48z8c_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = dcs6500_48z8c_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(dcs6500_48z8c_cpld_write);

static struct i2c_driver dcs6500_48z8c_cpld_driver = {
	.driver		= {
		.name	= "dcs6500_48z8c_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= dcs6500_48z8c_cpld_probe,
	.remove		= dcs6500_48z8c_cpld_remove,
	.id_table	= dcs6500_48z8c_cpld_id,
};

static int __init dcs6500_48z8c_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&dcs6500_48z8c_cpld_driver);
}

static void __exit dcs6500_48z8c_cpld_exit(void)
{
...
module_exit(dcs6500_48z8c_cpld_exit);
