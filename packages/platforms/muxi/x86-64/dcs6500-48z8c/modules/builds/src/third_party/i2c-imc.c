...
static void imc_instantiate_sensors(struct i2c_adapter *adapter, u8 presence)
{
	...
	strcpy_s(info.type, 20, DIMM_SENSOR_DRV);
...
static int imc_init_channelpair(struct imc_priv *priv, int i, int socket)
{
	...
	cp->adapter.nr = 200 + i; // Use 200/201 as the designated I2C bus.
	...
	snprintf_s(cp->adapter.name, sizeof(cp->adapter.name), sizeof(cp->adapter.name),
