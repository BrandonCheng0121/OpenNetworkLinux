...
#define SWITCH_MXSONIC_SWITCH_VERSION "0.0.0.1"

/* For MXSONiC */
static struct kobject *mx_switch_kobj;
EXPORT_SYMBOL_GPL(mx_switch_kobj);

static int __init switch_switch_init(void)
{
    /* For MXSONiC */
    mx_switch_kobj = kobject_create_and_add("switch", NULL);
    if(!mx_switch_kobj)
    {
...
MODULE_DESCRIPTION("Huarong Switch MXSONiC Switch Entry Driver");
MODULE_VERSION(SWITCH_MXSONIC_SWITCH_VERSION);
MODULE_LICENSE("GPL");
