...
// For KS SONiC optoe access
#define MAX_SWITCH_OPTOE_PORT 56
...
int optoe_bin_read_by_index(unsigned int index, char *buf, loff_t off, size_t len)
{
    ...
    for(temp_optoe_index=0; temp_optoe_index < MAX_SWITCH_OPTOE_PORT; temp_optoe_index++)
    {
        ...
        if(strncmp(s3ip_optoe_data[temp_optoe_index]->port_name,ETH_NAME_STRING,3))
        {
            continue;
        }
    ...
    return optoe_read_write(s3ip_optoe_data[temp_optoe_index], buf, off, len, OPTOE_READ_OP);
}
...
int optoe_bin_write_by_index(unsigned int index, char *buf, loff_t off, size_t len)
{
    ...
    return optoe_read_write(s3ip_optoe_data[temp_optoe_index], buf, off, len, OPTOE_WRITE_OP);
}
