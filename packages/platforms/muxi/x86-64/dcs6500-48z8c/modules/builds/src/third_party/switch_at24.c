...
struct at24_data *s3ip_at24[MAX_EEPROM_NUM] = {NULL};
...
static const struct i2c_device_id at24_ids[] = {
    ...
    { "switch_24c256",  AT24_DEVICE_MAGIC(262144 / 8,   AT24_FLAG_ADDR16) },
    { "switch_24c64",  AT24_DEVICE_MAGIC(65536 / 8,    AT24_FLAG_ADDR16) },
                     ...
                     unsigned int offset, size_t count)
{
    ...
    msg.buf[i++] = offset;
    if(memcpy_s(&msg.buf[i], io_limit+2, buf, count) != 0)
    {
        return -ENOMEM;
    }
    msg.len = i + count;
...
ssize_t at24_read_eeprom(unsigned int adapter_nr, unsigned int addr, uint8_t *eeprom, unsigned int offset, unsigned int count)
{
    ...
    for(i=0; i<MAX_EEPROM_NUM; i++)
    {
        if(s3ip_at24[i] != NULL)
        {
            ...
            if((s3ip_client->adapter->nr == adapter_nr) && (s3ip_client->addr == addr))
            {
                at24 = s3ip_at24[i];
                break;
            }
