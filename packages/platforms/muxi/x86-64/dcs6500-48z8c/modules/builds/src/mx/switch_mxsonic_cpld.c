#include <linux/module.h>
#include <linux/types.h>
#include <linux/init.h>
#include <linux/slab.h>

#include "switch_cpld_driver.h"

#define SWITCH_MXSONIC_CPLD_VERSION "0.0.0.1"

#define CPLD0_FMEA_KOBJ_NUM 2
#define CPLD1_FMEA_KOBJ_NUM 1
#define CPLD2_FMEA_KOBJ_NUM 1
#define CPLD3_FMEA_KOBJ_NUM 1

unsigned int loglevel = 0;

struct cpld_attribute{
    struct attribute attr;
    ssize_t (*show)(struct kobject *obj, struct cpld_attribute *attr, char *buf);
    ssize_t (*store)(struct kobject *obj, struct cpld_attribute *attr, const char *buf, size_t count);
};

struct cpld_drivers_t *cb_func = NULL;

/* For MXSONiC */
extern struct kobject *mx_switch_kobj;
static struct kobject *cpld_kobj;
static struct kobject *cpld_index_kobj[CPLD_TOTAL_NUM];
/* For CPLD0 */
static struct kobject *cpld0_fmea_kobj[CPLD0_FMEA_KOBJ_NUM];
/* For CPLD1 */
static struct kobject *cpld1_fmea_kobj[CPLD1_FMEA_KOBJ_NUM];

static char *cpld0_fmea_kobj_name[CPLD0_FMEA_KOBJ_NUM] = {"selftest", "voltage"};
static char *cpld1_fmea_kobj_name[CPLD1_FMEA_KOBJ_NUM] = {"selftest"};

enum cpld_attrs {
    DEBUG,
    LOGLEVEL,
    REBOOT_CAUSE,
    NUM,
    ALIAS,
    TYPE,
    HW_VERSION,
    BOARD_VERSION,
    NUM_CPLD_ATTR,
};

enum cpld_fmea_attrs {
    RESET,
    STATUS,
    NUM_CPLD_FMEA_ATTR,
};

int get_cpld_index(struct kobject *kobj)
{
    int retval;
    unsigned int cpld_index;
    char cpld_index_str[2] = {0};

    if(memcpy_s(cpld_index_str, 2, (kobject_name(kobj)+4), 1) != 0)
    {
        return -ENOMEM;
    }
    retval = kstrtoint(cpld_index_str, 10, &cpld_index);
    if(retval == 0)
    {
        CPLD_DEBUG("cpld_index:%d \n", cpld_index);
    }
    else
    {
        CPLD_DEBUG("Error:%d, cpld_index:%s \n", retval, cpld_index_str);
        return -1;
    }

    return (cpld_index-CPLD_INDEX_START);
}

int get_cpld_index_from_fmea_kobj(struct kobject *kobj)
{
    int retval;
    unsigned int cpld_index;
    char cpld_index_str[2] = {0};

    if(memcpy_s(cpld_index_str, 2, (kobject_name(kobj->parent)+4), 1) != 0)
    {
        return -ENOMEM;
    }
    retval = kstrtoint(cpld_index_str, 10, &cpld_index);
    if(retval == 0)
    {
        CPLD_DEBUG("cpld_index:%d \n", cpld_index);
    }
    else
    {
        CPLD_DEBUG("Error:%d, cpld_index:%s \n", retval, cpld_index_str);
        return -1;
    }

    return (cpld_index-CPLD_INDEX_START);
}

static ssize_t mxsonic_debug_help(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->debug_help(buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_debug(struct kobject *kobj, struct cpld_attribute *attr, const char *buf, size_t count)
{
    return count;
}

static ssize_t mxsonic_get_loglevel(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    return sprintf_s(buf, PAGE_SIZE, "%d\n", loglevel);
}

static ssize_t mxsonic_set_loglevel(struct kobject *kobj, struct cpld_attribute *attr, const char *buf, size_t count)
{
    int retval;
    long lev;

    if(cb_func == NULL)
        return -1;

    retval = kstrtol(buf, 0, &lev);
    if(retval == 0)
    {
        CPLD_DEBUG("lev:%ld \n", lev);
    }
    else
    {
        CPLD_DEBUG("Error:%d, lev:%s \n", retval, buf);
        return -1;
    }

    loglevel = (unsigned int)lev;

    cb_func->set_loglevel(lev);

    return count;
}

static ssize_t mxsonic_get_reboot_cause(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->get_reboot_cause(buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_get_num(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    return sprintf_s(buf, PAGE_SIZE, "%d\n", CPLD_TOTAL_NUM);
}

static ssize_t mxsonic_get_alias(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;
    int cpld_index;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    cpld_index = get_cpld_index(kobj);
    if(cpld_index < 0)
    {
        CPLD_DEBUG("Get cpld index failed.\n");
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->get_alias(cpld_index, buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_get_type(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;
    int cpld_index;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    cpld_index = get_cpld_index(kobj);
    if(cpld_index < 0)
    {
        CPLD_DEBUG("Get cpld index failed.\n");
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->get_type(cpld_index, buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_get_hw_version(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;
    int cpld_index;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    cpld_index = get_cpld_index(kobj);
    if(cpld_index < 0)
    {
        CPLD_DEBUG("Get cpld index failed.\n");
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->get_hw_version(cpld_index, buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_get_board_version(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int retval;
    int cpld_index;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    cpld_index = get_cpld_index(kobj);
    if(cpld_index < 0)
    {
        CPLD_DEBUG("Get cpld index failed.\n");
        ERROR_RETURN_NA(-1);
    }

    retval = cb_func->get_board_version(cpld_index, buf);
    if(retval < 0)
    {
        ERROR_RETURN_NA(retval);
    }
    else
    {
        return retval;
    }
}

static ssize_t mxsonic_get_fmea_status(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    int len;
    int cpld_index;

    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }
    
    cpld_index = get_cpld_index_from_fmea_kobj(kobj);
    if(cpld_index < 0)
    {
        CPLD_DEBUG("Get cpld index failed.\n");
        ERROR_RETURN_NA(-1);
    }

    if(!strcmp(kobject_name(kobj), "selftest"))
    {
        len = cb_func->get_fmea_selftest_status(cpld_index, buf);
    }
    else if(!strcmp(kobject_name(kobj), "corrosion"))
    {
        len = cb_func->get_fmea_corrosion_status(cpld_index, buf);
    }
    else if(!strcmp(kobject_name(kobj), "voltage"))
    {
        len = cb_func->get_fmea_voltage_status(cpld_index, buf);
    }
    else if(!strcmp(kobject_name(kobj), "clock"))
    {
        len = cb_func->get_fmea_clock_status(cpld_index, buf);
    }
    else if(!strcmp(kobject_name(kobj), "battery"))
    {
        len = cb_func->get_fmea_battery_status(buf);
    }
    else;

    return len;
}

static ssize_t mxsonic_get_fmea_reset(struct kobject *kobj, struct cpld_attribute *attr, char *buf)
{
    if(cb_func == NULL)
    {
        ERROR_RETURN_NA(-1);
    }

    return cb_func->get_fmea_reset(buf);
}

static ssize_t mxsonic_set_fmea_reset(struct kobject *kobj, struct cpld_attribute *attr, const char *buf, size_t count)
{
    int retval;
    int reset;

    if(cb_func == NULL)
    {
        return -ENXIO;
    }

    retval = kstrtoint(buf, 10, &reset);
    if(retval == 0)
    {
        CPLD_DEBUG("reset:%d \n", reset);
    }
    else
    {
        CPLD_DEBUG("Error:%d, reset:%s \n", retval, buf);
        return -ENXIO;
    }

    retval = cb_func->set_fmea_reset(reset);
    if(retval == false)
    {
        CPLD_DEBUG("Set CPLD reset failed.\n");
        return -ENXIO;
    }

    return count;
}

static struct cpld_attribute cpld_attr[NUM_CPLD_ATTR] = {
    [DEBUG]                 = {{.name = "debug",                .mode = S_IRUGO | S_IWUSR},     mxsonic_debug_help,             mxsonic_debug},
    [LOGLEVEL]              = {{.name = "loglevel",             .mode = S_IRUGO | S_IWUSR},     mxsonic_get_loglevel,           mxsonic_set_loglevel},
    [REBOOT_CAUSE]          = {{.name = "reboot_cause",         .mode = S_IRUGO},               mxsonic_get_reboot_cause,       NULL},
    [NUM]                   = {{.name = "num",                  .mode = S_IRUGO},               mxsonic_get_num,                NULL},
    [ALIAS]                 = {{.name = "alias",                .mode = S_IRUGO},               mxsonic_get_alias,              NULL},
    [TYPE]                  = {{.name = "type",                 .mode = S_IRUGO},               mxsonic_get_type,               NULL},
    [HW_VERSION]            = {{.name = "hw_version",           .mode = S_IRUGO},               mxsonic_get_hw_version,         NULL},
    [BOARD_VERSION]         = {{.name = "board_version",        .mode = S_IRUGO},               mxsonic_get_board_version,      NULL},
};

static struct cpld_attribute cpld_fmea_attr[NUM_CPLD_FMEA_ATTR] = {
    [RESET]                 = {{.name = "reset",                .mode = S_IRUGO | S_IWUSR},     mxsonic_get_fmea_reset,                     mxsonic_set_fmea_reset},
    [STATUS]                = {{.name = "status",               .mode = S_IRUGO},               mxsonic_get_fmea_status,                    NULL},
};

void mxsonic_cpld_drivers_register(struct cpld_drivers_t *pfunc)
{
    cb_func = pfunc;

    return;
}
EXPORT_SYMBOL_GPL(mxsonic_cpld_drivers_register);

void mxsonic_cpld_drivers_unregister(void)
{
    cb_func = NULL;

    return;
}
EXPORT_SYMBOL_GPL(mxsonic_cpld_drivers_unregister);

static int __init switch_cpld_init(void)
{
    int err;
    int retval;
    int i,j;
    int cpld_index;
    char *cpld_index_str;

    cpld_index_str = (char *)kzalloc(7*sizeof(char), GFP_KERNEL);
    if (!cpld_index_str)
    {
        CPLD_DEBUG( "Fail to alloc cpld_index_str memory\n");
        return -ENOMEM;
    }

    /* For MXSONiC */
    cpld_kobj = kobject_create_and_add("cpld", mx_switch_kobj);
    if(!cpld_kobj)
    {
        CPLD_DEBUG( "Failed to create 'cpld'\n");
        err = -ENOMEM;
        goto sysfs_create_kobject_cpld_failed;
    }

    for(i=DEBUG; i <= NUM; i++)
    {
        CPLD_DEBUG( "sysfs_create_file /cpld/%s\n", cpld_attr[i].attr.name);
        retval = sysfs_create_file(cpld_kobj, &cpld_attr[i].attr);
        if(retval)
        {
            printk(KERN_ERR "Failed to create file '%s'\n", cpld_attr[i].attr.name);
            err = -retval;
            goto sysfs_create_mxsonic_attr_failed;
        }
    }

    for(cpld_index=0; cpld_index<CPLD_TOTAL_NUM; cpld_index++)
    {
        if(sprintf_s(cpld_index_str, 7, "cpld%d", cpld_index+CPLD_INDEX_START) < 0)
        {
            err = -ENOMEM;
            goto sysfs_create_kobject_switch_cpld_index_failed;
        }
        cpld_index_kobj[cpld_index] = kobject_create_and_add(cpld_index_str, cpld_kobj);
        if(!cpld_index_kobj[cpld_index])
        {
            CPLD_DEBUG( "Failed to create 'cpld%d'\n", cpld_index+CPLD_INDEX_START);
            err = -ENOMEM;
            goto sysfs_create_kobject_switch_cpld_index_failed;
        }

        for(i=ALIAS; i < NUM_CPLD_ATTR; i++)
        {
            CPLD_DEBUG( "sysfs_create_file /cpld/cpld%d/%s\n", cpld_index+CPLD_INDEX_START, cpld_attr[i].attr.name);
            retval = sysfs_create_file(cpld_index_kobj[cpld_index], &cpld_attr[i].attr);
            if(retval)
            {
                printk(KERN_ERR "Failed to create file '%s'\n", cpld_attr[i].attr.name);
                err = -retval;
                goto sysfs_create_mxsonic_cpld_attr_failed;
            }
        }

        if(cpld_index == 0)
        {
            for(i = 0; i < CPLD0_FMEA_KOBJ_NUM; i++)
            {
                cpld0_fmea_kobj[i] = kobject_create_and_add(cpld0_fmea_kobj_name[i], cpld_index_kobj[cpld_index]);
                if(!cpld0_fmea_kobj[i])
                {
                    CPLD_DEBUG( "Failed to create '%s'\n", cpld0_fmea_kobj_name[i]);
                    err = -ENOMEM;
                    goto sysfs_create_mxsonic_cpld_fmea_kobj_failed;
                }

                for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                {
                    CPLD_DEBUG( "sysfs_create_file /cpld/cpld%d/%s/%s\n", cpld_index, cpld0_fmea_kobj_name[i], cpld_fmea_attr[j].attr.name);
                    retval = sysfs_create_file(cpld0_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    if(retval)
                    {
                        printk(KERN_ERR "Failed to create file '%s'\n", cpld_fmea_attr[i].attr.name);
                        err = -retval;
                        goto sysfs_create_mxsonic_cpld_fmea_attr_failed;
                    }
                }
            }
        }
        else if(cpld_index == 1)
        {
            for(i = RESET; i < STATUS; i++)
            {
                CPLD_DEBUG( "sysfs_create_file /cpld/cpld%d/%s\n", cpld_index, cpld_fmea_attr[i].attr.name);
                retval = sysfs_create_file(cpld_index_kobj[cpld_index], &cpld_fmea_attr[i].attr);
                if(retval)
                {
                    printk(KERN_ERR "Failed to create file '%s'\n", cpld_fmea_attr[i].attr.name);
                    err = -retval;
                    goto sysfs_create_mxsonic_fmea_attr_reset_failed;
                }
            }
            for(i = 0; i < CPLD1_FMEA_KOBJ_NUM; i++)
            {
                cpld1_fmea_kobj[i] = kobject_create_and_add(cpld1_fmea_kobj_name[i], cpld_index_kobj[cpld_index]);
                if(!cpld1_fmea_kobj[i])
                {
                    CPLD_DEBUG( "Failed to create '%s'\n", cpld1_fmea_kobj_name[i]);
                    err = -ENOMEM;
                    goto sysfs_create_mxsonic_cpld_fmea_kobj_failed;
                }

                for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                {
                    CPLD_DEBUG( "sysfs_create_file /cpld/cpld%d/%s/%s\n", cpld_index, cpld1_fmea_kobj_name[i], cpld_fmea_attr[j].attr.name);
                    retval = sysfs_create_file(cpld1_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    if(retval)
                    {
                        printk(KERN_ERR "Failed to create file '%s'\n", cpld_fmea_attr[i].attr.name);
                        err = -retval;
                        goto sysfs_create_mxsonic_cpld_fmea_attr_failed;
                    }
                }
            }
        }
    }

    kfree(cpld_index_str);

    return 0;

sysfs_create_mxsonic_fmea_attr_reset_failed:
sysfs_create_mxsonic_cpld_fmea_attr_failed:
sysfs_create_mxsonic_cpld_fmea_kobj_failed:
sysfs_create_mxsonic_cpld_attr_failed:
sysfs_create_kobject_switch_cpld_index_failed:
    for(i=DEBUG; i <= NUM; i++)
        sysfs_remove_file(cpld_kobj, &cpld_attr[i].attr);

    for(cpld_index=0; cpld_index<CPLD_TOTAL_NUM; cpld_index++)
    {
        if(cpld_index_kobj[cpld_index])
        {
            for(i=ALIAS; i < NUM_CPLD_ATTR; i++)
                sysfs_remove_file(cpld_index_kobj[cpld_index], &cpld_attr[i].attr);

            if(cpld_index == 0)
            {
                for(i = 0; i < CPLD0_FMEA_KOBJ_NUM; i++)
                {
                    if(cpld0_fmea_kobj[i])
                    {
                        for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                            sysfs_remove_file(cpld0_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    }

                    kobject_put(cpld0_fmea_kobj[i]);
                    cpld0_fmea_kobj[i] = NULL;
                }
            }
            else if(cpld_index == 1)
            {
                for(i = RESET; i < STATUS; i++)
                    sysfs_remove_file(cpld_index_kobj[cpld_index], &cpld_fmea_attr[i].attr);
                for(i = 0; i < CPLD1_FMEA_KOBJ_NUM; i++)
                {
                    if(cpld1_fmea_kobj[i])
                    {
                        for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                            sysfs_remove_file(cpld1_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    }

                    kobject_put(cpld1_fmea_kobj[i]);
                    cpld1_fmea_kobj[i] = NULL;
                }
            }
            kobject_put(cpld_index_kobj[cpld_index]);
            cpld_index_kobj[cpld_index] = NULL;
        }
    }

sysfs_create_mxsonic_attr_failed:
    if(cpld_kobj)
    {
        kobject_put(cpld_kobj);
        cpld_kobj = NULL;
    }

sysfs_create_kobject_cpld_failed:
    kfree(cpld_index_str);

    return err;
}

static void __exit switch_cpld_exit(void)
{
    int i,j;
    int cpld_index;

    /* For MXSONiC */
    for(i=DEBUG; i <= NUM; i++)
        sysfs_remove_file(cpld_kobj, &cpld_attr[i].attr);

    for(cpld_index=0; cpld_index<CPLD_TOTAL_NUM; cpld_index++)
    {
        if(cpld_index_kobj[cpld_index])
        {
            for(i=ALIAS; i < NUM_CPLD_ATTR; i++)
                sysfs_remove_file(cpld_index_kobj[cpld_index], &cpld_attr[i].attr);

            if(cpld_index == 0)
            {
                for(i = 0; i < CPLD0_FMEA_KOBJ_NUM; i++)
                {
                    if(cpld0_fmea_kobj[i])
                    {
                        for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                            sysfs_remove_file(cpld0_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    }

                    kobject_put(cpld0_fmea_kobj[i]);
                    cpld0_fmea_kobj[i] = NULL;
                }
            }
            else if(cpld_index == 1)
            {
                for(i = RESET; i < STATUS; i++)
                    sysfs_remove_file(cpld_index_kobj[cpld_index], &cpld_fmea_attr[i].attr);
                for(i = 0; i < CPLD1_FMEA_KOBJ_NUM; i++)
                {
                    if(cpld1_fmea_kobj[i])
                    {
                        for(j = STATUS; j < NUM_CPLD_FMEA_ATTR; j++)
                            sysfs_remove_file(cpld1_fmea_kobj[i], &cpld_fmea_attr[j].attr);
                    }

                    kobject_put(cpld1_fmea_kobj[i]);
                    cpld1_fmea_kobj[i] = NULL;
                }
            }

            kobject_put(cpld_index_kobj[cpld_index]);
            cpld_index_kobj[cpld_index] = NULL;
        }
    }

    if(cpld_kobj)
    {
        kobject_put(cpld_kobj);
        cpld_kobj = NULL;
    }

    cb_func = NULL;

    return;
}

MODULE_DESCRIPTION("Muxi Switch MXSONiC CPLD Driver");
MODULE_VERSION(SWITCH_MXSONIC_CPLD_VERSION);
MODULE_LICENSE("GPL");

module_init(switch_cpld_init);
module_exit(switch_cpld_exit);